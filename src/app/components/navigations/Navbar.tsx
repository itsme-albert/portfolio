            ...
            <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 bg-black text-xl">
              {navItems.map((item, index) => (
                <li key={index} className='flex items-center'>
                  {item.icon}
                  <a href={item.href} className="block py-2 text-white px-3 rounded-sm hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">
                    {item.name}
                  </a>
                </li>
              ))}
